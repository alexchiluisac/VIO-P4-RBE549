...
f = open("results/stamped_traj_estimate.txt", "w")
...
class MSCKF(object):
    ...
    def initialize_gravity_and_bias(self):
        ...
        #adding all the angular velocity msg
    ...
    def process_model(self, time, m_gyro, m_acc):
        ...
        C_q = to_rotation(imu_state.orientation) #R_w_i

        F[:3, :3] = -skew(gyro)
        F[:3, 3:6] = -np.identity(3)
        F[6:9, :3] = -C_q.T @ skew(acc)
        F[6:9, 9:12] = -C_q.T
    ...
    def predict_new_state(self, dt, gyro, acc):
        ...
        if gyroNorm > 1e-5:
            dq_dt = (np.cos(gyroNorm*dt*0.5) * np.identity(4) +
                np.sin(gyroNorm*dt*0.5)/gyroNorm * Omega) @ q
            dq_dt2 = (np.cos(gyroNorm*dt*0.25) * np.identity(4) +
                np.sin(gyroNorm*dt*0.25)/gyroNorm * Omega) @ q
        else:
            dq_dt = np.cos(gyroNorm*dt*0.5) * (np.identity(4) +
                Omega*dt*0.5) @ q
            dq_dt2 = np.cos(gyroNorm*dt*0.25) * (np.identity(4) +
